...
export default function RouteOverlay({
  ...
  const antSegs = useMemo(
    () => makeAntSegs(phase),
    [phase, path, cum],
  );

  // /* ✅ 경로 전체가 보이도록 자동 확대/이동 */
  // useEffect(() => {
  //   console.log('🟣 RouteOverlay path length =', path.length);

  //   if (!mapRef.current) {
  //     console.log('⛔ mapRef.current 없음');
  //     return;
  //   }
  ...
  //   console.log('📌 setBounds 호출', bounds);
  //   mapRef.current.setBounds(bounds);
  // }, [path]);

  return (
    <Map
      center={center} // 타입 요구사항 충족용, 기본 기준점
      level={4}
